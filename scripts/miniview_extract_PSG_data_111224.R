...
# Set X1 as row names
rownames(PSG_sleep_vars) <- PSG_sleep_vars$X1

# Filter rows based on desired row names
PSG_sleep_vars <- PSG_sleep_vars[c(
  "Sleep Onset SO - latency", 
  "N1-latency (LO)", 
  "N2-latency (LO)", 
  "N3-latency (LO)", 
  "REM-latency (SO)", 
  ...
  "Sleep efficiency (TST/SPT)"
), ]


colnames(PSG_sleep_vars) <- c("description", "text_output")
PSG_sleep_vars <- PSG_sleep_vars[!(PSG_sleep_vars$description == ""), ]
...
output_jpeg_path <- paste0("Z:\\Polysomnography\\Data\\", substr(studyID, 1, 7),"\\",studyID, "_hyponogram.png")
...
# Fill 'studyID' for all rows
OSA_data$studyID <- as.character(studyID)
...
write.table(PSG_parameters, file = "Z:/Polysomnography/Sleep metrics/psg_parameters.csv", 
